...
export class TeamFinder {
  ...
  protected readonly suggestions = signal<Record<EntityFacet, EntityFacetOption[]>>({
    league: [],
    country: [],
  });
  protected readonly labels = signal<Record<EntityFacet, Record<string, FilterDisplay>>>({
    league: {},
    country: {},
  });
  ...
  protected async suggest(facet: EntityFacet, event: Event): Promise<void> {
  ...
  protected addFacet(facet: EntityFacet, option: EntityFacetOption, input: HTMLInputElement): void {
    if (facet === 'league')
      this.request.update((value) => ({
        ...value,
  ...
  protected removeFacet(facet: EntityFacet, key: string): void {
    if (facet === 'league')
      this.request.update((value) => ({
        ...value,
